...
public class NotificationConsumer {
    ...
    @KafkaListener(topics = "order-creating", groupId = "order-creating-group")
    public void listenOrderCreating(OrderRequest orderRequest) {
        notificationService.sendMailAboutOrder(orderRequest);
    }

    @KafkaListener(topics = "order-status-changed", groupId = "order-status-changed-group")
    public void listenOrderStatusChanged(OrderRequest orderRequest) {
        notificationService.sendMailAboutOrder(orderRequest);
    }

    @KafkaListener(topics = "payment-events", groupId = "payment-events-group")
